...
namespace Atavism
{
    ...
    public class StandardCommands : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            AtavismCommand.RegisterCommandHandler("showCurrency", HandleShowCurrency);
            
        }

        private void HandleShowCurrency(string args)
        {
            string msg = "";
            foreach (var currId in Inventory.Instance.Currencies.Keys)
            {
                msg += Inventory.Instance.Currencies[currId].Current + " " + Inventory.Instance.GetCurrencyName(currId)+"\n";
            }

            ClientAPI.Write("Currencies:\n"+msg);
        }
