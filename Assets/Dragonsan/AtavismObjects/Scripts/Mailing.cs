...
namespace Atavism
{
    ...
    public class MailEntry
    {
        ...
        public Dictionary<Currency, long> currencies = new Dictionary<Currency, long>();
    ...
    public class Mailing : MonoBehaviour
    {
        ...
        public void HandleMailList(Dictionary<string, object> props)
        {
            ...
            int unreaded = 0;
            if (mailList != null)
                prevCount = mailList.Count;
            mailList = new List<MailEntry>();
            int numMail = (int)props["numMail"];
            
          //  Debug.LogError("Mail numMail=" + numMail);
            for (int i = 0; i < numMail; i++)
            {
              ...
              //  Debug.LogError("entry.mailId="+ entry.mailId+" s="+ entry.subject+ " numItems="+ numItems);
                ...
                long currencyAmount = (long)props["mail_" + i + "CurrencyAmount"];
                if (currencyType > 0)
            ...
            if (prevCount < mailList.Count && unreaded > 0)
            {
                string[] args1 = new string[1];
                AtavismEventSystem.DispatchEvent("NEW_MAIL", args1);
            }

            if (unreaded == 0)
            {
                string[] args1 = new string[1];
                AtavismEventSystem.DispatchEvent("NO_NEW_MAIL", args1);
            }
            //dispatch an event to tell the rest of the system
            string[] args = new string[1];
