...
namespace Atavism
{
    ...
    public class AtavismSocial : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            NetworkAPI.RegisterExtensionMessageHandler("privateInvite", HandlePrivateInvite);
        }
        ...
        public void PrivateInviteResponse(object inviter, bool accepted)
        {
            SendInvitePrivateResponseMessage((OID)inviter, accepted);
           
        }

        #region Message Senders
                
        public void SendInvitationToPtivate(OID friendOid)
        {
            Dictionary<string, object> props = new Dictionary<string, object>();
            props.Add("targetOid", friendOid);
            NetworkAPI.SendExtensionMessage(ClientAPI.GetPlayerOid(), false, "social.PRIVATE_INVITE", props);
        }

        public void SendInvitePrivateResponseMessage(OID targetOid, bool response)
        {
            Dictionary<string, object> props = new Dictionary<string, object>();
            props.Add("inviterOid", targetOid);
            props.Add("response", response);
            NetworkAPI.SendExtensionMessage(ClientAPI.GetPlayerOid(), false, "social.PRIVATE_INVITE_RESPONSE", props);
        }
        ...
        public void HandlePrivateInvite(Dictionary<string, object> props)
        {
            OID inviterOid = (OID)props["inviterOid"];
            string inviterName = (string)props["inviterName"];
            int timeOut = (int)props["timeOut"];
#if AT_I2LOC_PRESET
		string inviteMessage = inviterName + " "+I2.Loc.LocalizationManager.GetTranslation("has invited you to join Private Instance");
#else

            string inviteMessage = inviterName + " has invited you to join Private Instance";
#endif
            UGUIConfirmationPanel.Instance.ShowConfirmationBox(inviteMessage, inviterOid, PrivateInviteResponse,timeOut);
        }
