...
namespace Atavism
{
    ...
    public class AtavismMobName : MonoBehaviour
    {
            ...
            while (Camera.main != null)
            {
                ...
                if (mobQuest != null)
                {
                    ...
                    if (distance < renderDistance)
                    {
                        ...
                        {
                            if (mobQuest.GetComponent<TextMeshPro>() != null)
                                mobQuest.GetComponent<TextMeshPro>().enabled = true;
                        }
                    }
                    else if (mobQuest.GetComponent<TextMeshPro>() != null)
                        mobQuest.GetComponent<TextMeshPro>().enabled = false;
                }
                yield return delay;
            }
        ...
        void UpdateNameDisplay(bool showName)
        {
            ...
            string level = "";
            if (AtavismSettings.Instance.ShowLevelWithName)
            {
                if (node.PropertyExists("level"))
#if AT_I2LOC_PRESET
                    if (!string.IsNullOrEmpty(I2.Loc.LocalizationManager.GetTranslation("level")))
                        level = " ( " + I2.Loc.LocalizationManager.GetTranslation("level") + " " + node.GetProperty("level") + " )";
                    else
                         level = " ( level "+node.GetProperty("level")+" )";
#else
                    level = " ( level " + node.GetProperty("level") + " )";
#endif
            }
            ...
            if (showName && node != null)
            {
                ...
                text = shopTitle+"\n"+text + mName + level+ ownerName + "</color>" + SubTitle + title+ guildName;
