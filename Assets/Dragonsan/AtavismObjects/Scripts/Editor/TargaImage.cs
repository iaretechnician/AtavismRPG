...
namespace Atavism
{
...

    {
        ...
        private void LoadTGAImage(BinaryReader binReader)
        {
            ...
            tex = new Texture2D((int)this.objTargaHeader.Width, (int)this.objTargaHeader.Height);
            Color32[] pulledColors = new Color32[this.objTargaHeader.Width * this.objTargaHeader.Height];
             for (int i = 0; i <  this.objTargaHeader.Height; i++)
             {
                for (int j = 0; j < this.objTargaHeader.Width; j++)
                {
                    int k = i * this.objTargaHeader.Width + j;
                   // Debug.LogWarning(k);
                    pulledColors[(this.objTargaHeader.Height-1-i)*this.objTargaHeader.Width + j] = new Color32(bimagedata[k * 4 + 2], bimagedata[k * 4 + 1], bimagedata[k * 4 + 0], bimagedata[k * 4 + 3]);
                }
            }

             // for (int i = 0; i <  this.objTargaHeader.Height*this.objTargaHeader.Width; i++)
            //     pulledColors[i] = new Color32(bimagedata[i*4+2], bimagedata[i*4+1], bimagedata[i*4+0], bimagedata[i*4+3]);
             //   pulledColors[pulledColors.Length-1-i] = new Color32(bimagedata[i*4+2], bimagedata[i*4+1], bimagedata[i*4+0], bimagedata[i*4+3]);
...
*/
        ...
        // public Bitmap LoadTargaImage(string sFileName)
        // {
        ...
        //     }
        // }
        public Texture2D LoadTargaImage(string sFileName)
        {
            using (TargaImage ti = new TargaImage(sFileName))
            {
                return ti.tex;
