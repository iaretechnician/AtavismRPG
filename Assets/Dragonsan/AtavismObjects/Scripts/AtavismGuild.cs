...
namespace Atavism
{
    ...
    public class AtavismGuild : MonoBehaviour
    {
        ...
        int level = 0;
        int memberNum = 0;
        Dictionary<int, int> reqItems = new Dictionary<int, int>();
        ...
        void Start()
        {
            ...
            NetworkAPI.RegisterExtensionMessageHandler("guildResource", HandleGuildResource);
        ...
        public void SendGuildResource(int itemId, int count)
        {
            Dictionary<string, object> props = new Dictionary<string, object>();
            props.Add("itId", itemId);
            props.Add("itCount", count);
            NetworkAPI.SendExtensionMessage(ClientAPI.GetPlayerOid(), false, "guild.addResource", props);
        }
		...
		//ClientAPI.Write(I2.Loc.LocalizationManager.GetTranslation("Got guild update"));
            ...
            for (int i = 0; i < numRanks; i++)
            {
                ...
                for (int j = 0; j < rankNumPermissions; j++)
                {
                 ...
                 //   Debug.LogError("GUILD rank.rankName="+ rank.rankName+" permission = " + permission);
                    rank.permissions.Add((AtavismGuildRankPermission)Enum.Parse(typeof(AtavismGuildRankPermission), permission));
                }
                ranks.Add(rank);
            }
            string ite = (string)props["resource"];
         //   Debug.LogError("GUILD its " + ite);
            string[] it = ite.Split('|');
            items.Clear();
            for (int i = 0; i < it.Length / 2; i++)
            {
                items.Add(int.Parse(it[2 * i]), int.Parse(it[2 * i + 1]));
            }
            string rItems = (string)props["req"];
        //    Debug.LogError("GUILD req " + rItems);
            string[] rit = rItems.Split('|');
            reqItems.Clear();
            for (int i = 0; i < rit.Length / 2; i++)
            {
                reqItems.Add(int.Parse(rit[2 * i]), int.Parse(rit[2 * i + 1]));
            }
            if(props.ContainsKey("memberNum"))
            memberNum = (int)props["memberNum"];
            // dispatch a ui event to tell the rest of the system
        ...
        public void HandleGuildResource(Dictionary<string, object> props)
        {
            reqItems.Clear();
            items.Clear();
          ...
          //  Debug.LogError("GUILD req " + rItems);
            ...
            // dispatch a ui event to tell the rest of the system
            string[] event_args = new string[1];
            AtavismEventSystem.DispatchEvent("GUILD_RES_UPDATE", event_args);
        }
