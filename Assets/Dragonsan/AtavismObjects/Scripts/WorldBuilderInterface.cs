...
namespace Atavism
{
    ...
    public class WorldBuilderInterface : MonoBehaviour
    {
                    ...
                    if (BuildManager.Instance.Pieces.Count != 0)
                    {
                        if (BuilderBehaviour.Instance != null)
                        {
                            ...
                            foreach (var piece in BuildManager.Instance.Pieces)
                            {
                                if (piece != null)
                                {
                                    AtavismLogger.LogDebugMessage("BuildObjectClicked piece"+piece.Id+" " + buildObjectID + " piece.BuildObjDefId=" + piece.BuildObjDefId);
                                    if (piece.BuildObjDefId == buildObjectID)
                                    {
                                        AtavismLogger.LogDebugMessage("BuildObjectClicked found " + buildObjectID + " piece.BuildObjDefId=" + piece.BuildObjDefId+" "+piece.name);
                            ...
                            AtavismLogger.LogDebugMessage("BuildObjectClicked " + buildObjectID + " found=" + f);
                            BuilderBehaviour.Instance.ChangeMode(BuildMode.Placement);
                        }
                        else
                        {
                            WorldBuilder.Instance.StartPlaceBuildObject(buildObjectID);
                        }
                    }
        ...
        public AtavismInventoryItem ItemBeingPlaced()
        {
            return itemBeingPlaced;
        }
    }
}