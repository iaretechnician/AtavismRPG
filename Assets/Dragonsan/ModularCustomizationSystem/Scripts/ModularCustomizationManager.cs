...
namespace HNGamers
{
    ...
    namespace Atavism
    {
        ...
        public class ModularCustomizationManager : MonoBehaviour
        {
            ...
            [Header("Character Model Transforms")]
            /*
            ...
            */
            ...
            [Tooltip("Select the head slot that will be used for changing the item. This is synced to the skin materials. This is not a multipurpse slot.")]
            public string headSlot;

            [Tooltip("Select the torso slot that will be used for changing the item. This is synced to the skin materials. This is not a multipurpse slot.")]
            public string torsoSlot;
            [Tooltip("Select the upper arm slot that will be used for changing the item. This is synced to the skin materials. This is not a multipurpse slot.")]
            public string upperarmSlot;
            [Tooltip("Select the lower arm slot that will be used for changing the item. This is synced to the skin materials.")]
            public string lowerarmSlot;
            [Tooltip("Select the hips slot that will be used for changing the item. This is synced to the skin materials.")]
            public string hipSlot;
            [Tooltip("Select the upper leg slot that will be used for changing the item. This is synced to the skin materials.")]
            public string upperlegSlot;
            [Tooltip("Select the lower leg slot that will be used for changing the item. This is synced to the skin materials.")]
            public string lowerlegSlot;
            [Tooltip("Select the hands slot that will be used for changing the item. This is synced to the skin materials.")]
            public string handsSlot;
            [Tooltip("Select the feet slot that will be used for changing the item. This is synced to the skin materials.")]
            public string feetSlot;
            [Tooltip("Select the parent slot that will be used for targeting the hair location. This is not a multipurpse slot.")]
            public string parentSlot;
            [Tooltip("Select the torso slot that will be used for changing the root of the object. This is not a multipurpse slot.")]
...
#if ModularNHance
                if (defaultLegs && nHanceModelSwapping)
                {
                    ActiveHips = enableNHance(defaultLegs);
                }

                if (defaultHands && nHanceModelSwapping)
                {
                    ActiveHands = enableNHance(defaultHands);
                }

                if (defaultFeet && nHanceModelSwapping)
                {
                    ActiveFeet = enableNHance(defaultFeet);
                }

                if (defaultTorso && nHanceModelSwapping)
                {
            ...
            public List<GameObject> enableNHance(GameObject thisobject)
            {
                ...
                m_nHanceReplacementItem = (nHanceReplacementItem)Resources.Load("nHanceReplacementItems\\" + thisobject.name); //Add this line
            ...
            public void UpdateBeardModel(int hairItem)
            {
                ...
                else
                {
                    ...
                    if (beardPrefab == null)
                        return;
                    ActiveBeardId = hairItem;
                    ActiveBeard = (GameObject)Instantiate(beardPrefab, atavismMobAppearance.GetSocketTransform(beardSlot).position, atavismMobAppearance.GetSocketTransform(beardSlot).rotation);
            ...
            public void UpdateHairModel(int hairItem)
            {
                ...
                else
                {
                    ...
                    ActiveHairId = hairItem;
                    ActiveHair = (GameObject) Instantiate(prefab, atavismMobAppearance.GetSocketTransform(parentSlot).position, atavismMobAppearance.GetSocketTransform(parentSlot).rotation);
...
//Debug.LogError("Torso Clear");
            ...
            public void ChangeBody(ModularReplacementItem targetBody)
            {
                ...
                foreach (ReplacementItems thisItem in targetBody.replacementItems)
                {
                    ...
                    {
                        Transform thisobject = FindDeepChild(targetBody.fullModelReplacement.transform, targetBody.fullModelReplacement.name);
                        if (!thisobject && targetBody.FullModelReplacement.Length >= 1)
                        {
                            thisobject = FindDeepChild(targetBody.fullModelReplacement.transform, targetBody.FullModelReplacement);
                        }
            ...
            private List<SkinnedMeshRendererChildObjectDictionary>  skinnedMeshRendererChildObjectDictionary = new List<SkinnedMeshRendererChildObjectDictionary>();
            ...
            private void SwapSkinnedMeshRenderer(SkinnedMeshRenderer currentSkin, SkinnedMeshRenderer targetSkin)
            {
                ...
                for (int i = 0; i < targetSkin.bones.Length; i++)
                {
                    ...
                    {
                        Debug.LogWarningFormat("Model has Extra bones that have not been accounted for at transform {0} with name {1}",newMeshBones[i].name,bone.name);
            ...
            private void SpawnChildenObjects(List<SkinnedMeshRendererChildObjectDictionary> thisobject, Transform thisObjectsParentTranform)
            {
                foreach (var childObjectSpawned in thisobject)
                {
                    Debug.LogWarningFormat("SpawnChildenObjects at transform {0} with name {1}",  childObjectSpawned.thisTransform.name, childObjectSpawned.thisGameObject.name);
                    childObjectSpawned.thisGameObject.SetActive(true);
                    GameObject thisChildGameObject = Instantiate(childObjectSpawned.thisGameObject, thisObjectsParentTranform);
                    ...
                    thisChildGameObject.gameObject.transform.localScale = childObjectSpawned.thisGameObject.transform.localScale;
            ...
            private void DestroyChildenObjects(List<SkinnedMeshRendererChildObjectDictionary> thisobject)
            {
                ...
                {
                    List<SkinnedMeshRendererChildObjectDictionary> SkinnedMeshRendererChildObjectDictionaryCopy = new List<SkinnedMeshRendererChildObjectDictionary>(skinnedMeshRendererChildObjectDictionary);
                ...
                if (skinnedMeshRendererChildObjectDictionarySpawned.Count > 0)
                {
                    ...
                    List<GameObject> skinnedMeshRendererChildObjectDictionarySpawnedCopy = new List<GameObject>(skinnedMeshRendererChildObjectDictionarySpawned);
                    ...
                    if (childObjectMeshRenderer.Length >= 1)
                    {
                        foreach (var item in childObjectMeshRenderer)
                        {
                            ...
                            if (item.gameObject.activeSelf)
                                skinnedMeshRendererChildObjectDictionary.Add(thisSkinnedMeshRendererChildObjectDictionary );
                        }
                    }
                }
            }
