    ...
    public class bl_MiniMapEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
           bl_MiniMap script = (bl_MiniMap)target;
            ...
            EditorGUILayout.LabelField("UGUI Mini Map for Atavism");

            EditorGUILayout.BeginVertical("box");
            if (GUILayout.Button("General Settings", EditorStyles.toolbarPopup))
            {
                ShowGeneral = !ShowGeneral;
                GeneralAnim.target = ShowGeneral;
            }
...
#if AT_I2LOC_PRESET
                    EditorGUILayout.LabelField("Map Texture For Language");
                    if (script.MapTextures == null)
                        script.MapTextures = new System.Collections.Generic.List<MapLang>();
                    for (int i = 0; i < script.MapTextures.Count; i++)
                    {
                        script.MapTextures[i].name = EditorGUILayout.TextField("Language Name", script.MapTextures[i].name);
                        script.MapTextures[i].mapTexture = EditorGUILayout.ObjectField("Map Texture for "+ script.MapTextures[i].name, script.MapTextures[i].mapTexture, typeof(Texture), allowSceneObjects) as Texture;
                        
                    }
                    EditorGUILayout.BeginHorizontal();
                    if (GUILayout.Button("Add"))
                    {
                        script.MapTextures.Add(new MapLang());
                    }
                    if (GUILayout.Button("Remove"))
                    {
                        if (script.MapTextures.Count > 0)
                            script.MapTextures.RemoveAt(script.MapTextures.Count - 1);
                    }
            ...
            if (GUILayout.Button("Position Settings", EditorStyles.toolbarPopup))
            {
                ...
                PositionAnim.target = ShowPosition;
            }
            ...
            if (GUILayout.Button("Rotation Settings", EditorStyles.toolbarPopup))
            {
                ...
                RotationAnim.target = ShowRotation;
            }
            ...
            if (GUILayout.Button("Animations Settings", EditorStyles.toolbarPopup))
            {
                ...
                AnimationsAnim.target = ShowAnimation;
            }
            ...
            if (GUILayout.Button("References", EditorStyles.toolbarPopup))
            {
                ...
                ReferencesAnim.target = ShowReferences;
            }
            if (EditorGUILayout.BeginFadeGroup(ReferencesAnim.faded))
            {
                ...
                script.PlayerCamera = EditorGUILayout.ObjectField("Player Camera Icon", script.PlayerCamera, typeof(Image), allowSceneObjects) as Image;
