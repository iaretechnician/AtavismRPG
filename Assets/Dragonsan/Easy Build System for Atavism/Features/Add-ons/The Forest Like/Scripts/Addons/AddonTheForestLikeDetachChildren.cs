...
[Addon("The Forest Detach Children", AddonTarget.PieceBehaviour)]
public class AddonTheForestLikeDetachChildren : AddonBehaviour
{
    ...
    private void Update()
    {
        if (Piece.GetComponent<InternalPhysicsCondition>() == null) return;

        if (Piece.GetComponent<InternalPhysicsCondition>().AffectedByPhysics)
        {
            if (IsExiting)
                return;
    ...
    private void OnPieceDestroyed(PieceBehaviour piece)
    {
        ...
        if (!Piece.GetComponent<InternalPhysicsCondition>().CheckStability())
            DetachChildren();
    }
